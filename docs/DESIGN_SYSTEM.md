...
## Phase 3 — 모션·디테일 컴포넌트 (적용 완료)

추가 4종 컴포넌트가 `src/ui/components/`에 합류.

### Tabs

```ts
import { createTabs } from '@/ui';

const tabs = createTabs(this, {
  x: 24, y: 76, w: 552,
  items: [
    { id: 'T1', label: 'T1 0/8' },
    { id: 'T2', label: 'T2 0/8' },
...
active 인디케이터(하단 2px stripe), pressTap 자동.
...
showToast(this, '🎉 R&D 완료: 테스트 자동화', { tone: 'ok' });
showToast(this, '파산 위험: 골드 0', { tone: 'bad', durationMs: 3000, position: 'bottom' });
```

자동 stack — 동시 호출 시 위로 쌓임. tone: neutral/ok/warn/bad.
...
confettiBurst(this, 360, 640);                          // 기본 16개
confettiBurst(this, x, y, { count: 30, radius: 200 });
```

CATEGORY 팔레트 자동 사용. 의존성 추가 없이 GameObject Tween으로 구현.

### ScrollArea

```ts
import { createScrollArea } from '@/ui';

const sa = createScrollArea(this, {
  x: panelX + 20, y: cardsY0,
  w: cardW, h: viewportH,
...
마스크 + 드래그 + wheel 입력. 모바일/PC 모두 작동.

### 적용 사례

- R&D 연구 시작: confetti + 토스트 ("🔬 ... 연구 시작 (N주)")
- R&D 연구 완료: ResultScene 진입 시 자동 감지 → 순차 토스트 + confetti
- 시설 건설 / 시장 진출 / 자회사 인수: confetti + 토스트

이제 화면 어디서든 액션이 일어나면 시각·청각 피드백이 일관되게 발생한다.

## Phase 4 예고 (선택)

- 다른 씬(Assignment/Development/Stats/Settings)도 토큰·컴포넌트 적용
- 카운트업 애니메이션 적용 (골드 변화)
- AssignmentScene 직원 카드를 createCard로 통일
- 스플래시 화면 + 로고 추가
